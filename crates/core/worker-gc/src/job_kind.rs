/// The `kind` discriminator value for garbage collection jobs.
pub const JOB_KIND: &str = "gc";

/// Type-safe representation of the garbage collection job kind.
///
/// Serializes as the string [`JOB_KIND`] and only deserializes successfully
/// when the input matches that exact value.
#[derive(Debug)]
pub struct GcJobKind;

impl std::fmt::Display for GcJobKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(JOB_KIND)
    }
}

impl std::str::FromStr for GcJobKind {
...
impl serde::Serialize for GcJobKind {
...
impl<'de> serde::Deserialize<'de> for GcJobKind {
